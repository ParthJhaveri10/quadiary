...
const MediaCategoryCard = ({ title, icon, color, text, onClick }) => (
  ...
  >
    <div className="flex items-center mb-4">
      <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${themeStyles.gradients.media[title.toLowerCase()]} flex items-center justify-center shadow-lg shadow-[#66FCF1]/20`}>
        {icon}
      </div>
    ...
    <div className="mt-4 flex justify-between items-center">
      <div className="flex space-x-1">
        {[...Array(5)].map((_, i) => (
          <svg key={i} className={`w-4 h-4 ${i < 3 ? 'text-[#66FCF1]' : 'text-[#C5C6C7]/20'}`} fill="currentColor" viewBox="0 0 20 20">
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
          </svg>
        ))}
      </div>
      <button className="text-sm font-medium bg-[#C5C6C7]/10 hover:bg-[#C5C6C7]/20 text-[#C5C6C7] px-3 py-1 rounded-lg transition-colors duration-200">
        Explore
      </button>
    </div>
  </div>
